...
function typeset(expression: string): string {
...
function messageBox(message: Message): HTMLElement {
...
function api(query: {}) {
    ...
    return new Promise((resolve, reject) => requestPOST('php/canard.php', JSON.stringify({
        'query': query
    })).then(function (response: string) { resolve(JSON.parse(response)); }).catch(reject));
}

function dbGet(table: string, fields: string[], conditions: { [key: string]: string } = {}) {
...
function dbInsert(table: string, values: { [key: string]: string }, password: string) {
...
function dbUpdate(table: string, values: { [key: string]: string }, conditions: { [key: string]: string }, password: string) {
    return new Promise((resolve, reject) => requestPOST('/php/data.php', JSON.stringify({
        'table': table,
        'values': values,
        'conditions': conditions,
